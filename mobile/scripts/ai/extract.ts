  ...
  properties: {
    capsules: {
      ...
      items: {
        ...
        properties: {
          format: { type: "string", enum: ["narrative", "stat", "recall", "interactive"] },
...
function systemPrompt(s: Source): string {
  return [
    ...
    `Dado "${s.title}" (${s.type}), genera entre 8 y 12 cápsulas. Cada una es UNA idea autónoma que se entiende sola.`,
    ...
    `El usuario aprende esto porque: "${s.intent ?? "quiere entenderlo a fondo y aplicarlo"}".`,
    `Varía los formatos para que la mezcla enganche:`,
    `· narrative: 3 slides (hook contraintuitivo → develop con ejemplo concreto → twist que da la vuelta). La mayoría.`,
    `· stat: una cifra que golpea (figure) + claim + reveal con el giro. Úsalo cuando haya un dato potente.`,
    `· interactive: scenario + 2 choices con su outcome (sin correcto/incorrecto) + insight.`,
    `· recall: prompt de recuerdo + reveal. Solo 0-1, para ideas centrales.`,
...
export async function generatePack(source: Source): Promise<Capsule[]> {
  ...
  const message = await anthropic.messages.create({
    ...
    // structured outputs (§4): la forma se garantiza por API, no por prompt.
    output_config: { format: { type: "json_schema", schema: SCHEMA } },
  } as any);

  const block = (message.content as any[]).find((b) => b.type === "text");
  if (!block) throw new Error("Claude no devolvió texto");
  const { capsules } = JSON.parse(block.text) as { capsules: RawCapsule[] };
